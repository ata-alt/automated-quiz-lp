            ...
            points: [
                {
                    ...
                    description: "Why settle for standard when you can have the exceptional? Our showroom houses an unparalleled selection of luxury sofas, from timeless classics to contemporary masterpieces, all meticulously crafted by the world's finest artisans."
                },
                {
                    title: '2. Bespoke, Just for You',
                    description: "Luxury is personal. That's why we offer fully customisable designs—every stitch, curve, and cushion tailored to your exacting standards. Whether it's hand-stitched Italian leather or a sumptuous velvet that whispers elegance, we bring your vision to life."
                },
                {
                    title: '3. The Touch Test',
                    description: "A sofa should never be chosen from a screen. It's about how it feels—the depth of the cushioning, the smoothness of the upholstery, the perfect balance of support and indulgence. Our showroom invites you to experience craftsmanship in its truest form."
                },
                {
                    title: '4. Design Expertise, on Demand',
                    description: "Our team of seasoned interior designers are at your service, ready to refine, reimagine, and elevate your space with tailored advice and creative insight. Whether you're seeking a statement piece or an entire living room transformation, we make it effortless."
                },
                {
                    title: '5. Instant Gratification',
                    description: "Impatience is a virtue when it comes to interiors. With many of our luxury sofas available for immediate delivery, there's no need to wait months for perfection to arrive."
                }
            ],
                ...
                options: [
                    {
                        ...
                        image: '/site-assets/images/sofa-quiz/young-kids-energetic-messy-and-always-moving.jpg'
                    },
                    {
                        id: 'b',
                        text: 'Teenagers or young adults still at home',
                        image: '/site-assets/images/sofa-quiz/teenagers-or-young-adults-still-at-home.jpg'
                    },
                    {
                        id: 'c',
                        text: 'Just the two of us—calm and design-focused',
                        image: '/site-assets/images/sofa-quiz/just-the-two-of-us-calm-and-design-focused.jpg'
                    },
                    {
                        id: 'd',
...
function renderLuxurySofasPoints() {
    ...
    if (!container || !quizData.luxurySofasSection || !quizData.luxurySofasSection.points) return;

    container.innerHTML = quizData.luxurySofasSection.points.map((point, index) => `
...
function renderQuestions() {
    ...
    if (bannerMainHeading && quizData.bannerSection) {
        bannerMainHeading.value = quizData.bannerSection.mainHeading || 'Match Your Personality To A Luxury Sofa.';
        document.getElementById('bannerSubHeading').value = quizData.bannerSection.subHeading || 'Try Our AI Tool';
        ...
        if (quizData.bannerSection.backgroundImage) {
            ...
            desktopImage.innerHTML = '<div class="upload-text">📷 Change Desktop Banner</div>';
        }
        ...
        if (quizData.bannerSection.mobileImage) {
            ...
            mobileImage.innerHTML = '<div class="upload-text">📱 Change Mobile Banner</div>';
        }
    }
    ...
    if (showroomHeading && quizData.showroomSection) {
        showroomHeading.value = quizData.showroomSection.heading || 'The largest luxury sofa showroom in London';
        ...
        if (showroomImage && quizData.showroomSection.image) {
            ...
            showroomImage.innerHTML = '<div class="upload-text">📷 Click to change showroom image</div>';
        }
    }
    ...
    if (quizData.luxurySofasSection) {
        ...
        if (luxurySofasTitle) {
            luxurySofasTitle.value = quizData.luxurySofasSection.title || 'Luxury Sofas, Redefined';
        }
        ...
        if (luxurySofasSubtitle) {
            luxurySofasSubtitle.value = quizData.luxurySofasSection.subtitle || 'Why Visit Our Showroom?';
        }

        const luxurySofasConclusion = document.getElementById('luxurySofasConclusion');
        if (luxurySofasConclusion) {
            luxurySofasConclusion.value = quizData.luxurySofasSection.conclusion || '';
        }
    ...
    const designDisasterHeading = document.getElementById('designDisasterHeading');
    if (designDisasterHeading && quizData.designDisasterSection) {
        designDisasterHeading.value = quizData.designDisasterSection.heading || 'Avoid a design disaster.\nTalk to an expert.';
        document.getElementById('designDisasterButtonText').value = quizData.designDisasterSection.buttonText || 'Book now';
        document.getElementById('designDisasterButtonLink').value = quizData.designDisasterSection.buttonLink || '/book-a-showroom-visit.html';
    ...
    if (quizPromoHeading && quizData.quizPromoSection) {
        quizPromoHeading.value = quizData.quizPromoSection.heading || 'Take our lifestyle quiz & find the perfect sofa match.';
        document.getElementById('quizPromoButtonText').value = quizData.quizPromoSection.buttonText || 'Try our Sofa Matching Quiz';
        document.getElementById('quizPromoButtonLink').value = quizData.quizPromoSection.buttonLink || '#sofaquiz';
    ...
    return options.map((option, oIndex) => `
        <div class="option-card">
            ...
            <div class="image-upload-area ${option.image ? 'has-image' : ''}"
                 ...
                 style="${option.image ? `background-image: url('${option.image}')` : ''}"
                 onclick="document.getElementById('file-${qIndex}-${oIndex}').click()">
                <div class="upload-text">
                    ${option.image ? '📷 Click to change image' : '📤 Click to upload image'}
                </div>
            </div>
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            if (type === 'desktop') {
                ...
                imageArea.innerHTML = '<div class="upload-text">📷 Change Desktop Banner</div>';
            } else if (type === 'mobile') {
                quizData.bannerSection.mobileImage = imageData;
                const imageArea = document.getElementById('banner-mobile-image');
                imageArea.style.backgroundImage = `url('${imageData}')`;
                imageArea.style.backgroundSize = 'cover';
                imageArea.innerHTML = '<div class="upload-text">📱 Change Mobile Banner</div>';
            }
...
function handleShowroomImageUpload(input) {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            imageArea.style.backgroundImage = `url('${imageData}')`;
            imageArea.innerHTML = '<div class="upload-text">📷 Click to change showroom image</div>';
    ...
    quizData.gallerySection.images.forEach((item, index) => {
        ...
        container.innerHTML = `
            <div style="border: 1px solid #ddd; padding: 10px; border-radius: 8px; background: white;">
                <h4 style="color: #9c27b0; margin: 0 0 10px 0;">Image ${index + 1}</h4>
                <div class="image-upload-area ${item.src ? 'has-image' : ''}"
                     id="gallery-image-${index}"
                     style="height: 150px; margin-bottom: 10px; ${item.src ? `background-image: url('${item.src}')` : ''}"
...
function renderQuizPromoFeatures() {
    ...
    container.innerHTML = quizData.quizPromoSection.features.map((feature, index) => `
...
function renderQuizPromoImages() {
    ...
    container.innerHTML = quizData.quizPromoSection.images.map((image, index) => `
        <div style="position: relative;">
            <div class="image-upload-area ${image ? 'has-image' : ''}"
                 id="quizpromo-image-${index}"
                 style="height: 100px; ${image ? `background-image: url('${image}')` : ''}"
...
function handleImageUpload(qIndex, oIndex, input) {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            imageArea.innerHTML = '<div class="upload-text">📷 Click to change image</div>';
...
function saveQuiz() {
    ...
    if (window.currentProductKey && window.currentProductKey !== 'sofa') {
        ...
        if (productQuizzes[window.currentProductKey]) {
            ...
            if (bannerMainHeading) productQuizzes[window.currentProductKey].bannerContent.mainHeading = bannerMainHeading.value;
            if (bannerSubHeading) productQuizzes[window.currentProductKey].bannerContent.subHeading = bannerSubHeading.value;

            const showroomHeading = document.getElementById('showroomHeading');
            if (showroomHeading) productQuizzes[window.currentProductKey].showroomContent.heading = showroomHeading.value;

            const designDisasterHeading = document.getElementById('designDisasterHeading');
            const designDisasterButtonText = document.getElementById('designDisasterButtonText');
            const designDisasterButtonLink = document.getElementById('designDisasterButtonLink');
            if (designDisasterHeading) productQuizzes[window.currentProductKey].designExpertContent.heading = designDisasterHeading.value;
            if (designDisasterButtonText) productQuizzes[window.currentProductKey].designExpertContent.buttonText = designDisasterButtonText.value;
            if (designDisasterButtonLink) productQuizzes[window.currentProductKey].designExpertContent.buttonLink = designDisasterButtonLink.value;
            ...
            if (quizPromoHeading) productQuizzes[window.currentProductKey].quizPromoContent.heading = quizPromoHeading.value;
            if (quizPromoButtonText) productQuizzes[window.currentProductKey].quizPromoContent.buttonText = quizPromoButtonText.value;
            if (quizPromoButtonLink) productQuizzes[window.currentProductKey].quizPromoContent.buttonLink = quizPromoButtonLink.value;

            // Also update luxurySofasSection data if it exists in quizData
            if (quizData.luxurySofasSection) {
                productQuizzes[window.currentProductKey].luxurySofasSection = quizData.luxurySofasSection;
            }
            if (quizData.gallerySection) {
                productQuizzes[window.currentProductKey].gallerySection = quizData.gallerySection;
            }
            if (quizData.designDisasterSection) {
                productQuizzes[window.currentProductKey].designDisasterSection = quizData.designDisasterSection;
            }
            if (quizData.quizPromoSection) {
                productQuizzes[window.currentProductKey].quizPromoSection = quizData.quizPromoSection;
            }

            // TODO: Save updated data to database
            // localStorage.setItem('productQuizzes', JSON.stringify(productQuizzes));
            showStatus(`Product quiz saved successfully! (Database integration pending)`, 'success');
        }
    } else {
        // TODO: Save sofa quiz to database
        // localStorage.setItem('sofaQuizData', JSON.stringify(quizData));
        showStatus('Sofa quiz saved successfully! (Database integration pending)', 'success');
    }
...
function createNewProductQuiz() {
    ...
    productQuizzes[productKey] = {
        ...
        galleryContent: {
            images: [
                { image: '', description: `Luxury ${productName.toLowerCase()} in modern setting` },
                { image: '', description: `Elegant ${productName.toLowerCase()} design` },
                { image: '', description: `Designer ${productName.toLowerCase()} collection` }
            ]
        },
        designExpertContent: {
            heading: 'Avoid a design disaster.\\nTalk to an expert.',
...
function switchProductQuiz(productKey) {
    ...
    if (productKey === 'sofa') {
        ...
        document.getElementById('dashboardTitle').textContent = '🛋️ Sofa Quiz Content Management Dashboard';
    ...
    document.getElementById('dashboardTitle').textContent = `${product.emoji} ${product.name} Quiz Content Management Dashboard`;
...
function loadProductQuizData(productKey, product) {
    ...
    const bannerMainHeading = document.getElementById('bannerMainHeading');
    const bannerSubHeading = document.getElementById('bannerSubHeading');
    if (bannerMainHeading) bannerMainHeading.value = product.bannerContent.mainHeading;
    if (bannerSubHeading) bannerSubHeading.value = product.bannerContent.subHeading;
    ...
    const designDisasterButtonText = document.getElementById('designDisasterButtonText');
    const designDisasterButtonLink = document.getElementById('designDisasterButtonLink');
    if (designDisasterHeading) designDisasterHeading.value = product.designExpertContent.heading;
    if (designDisasterButtonText) designDisasterButtonText.value = product.designExpertContent.buttonText;
    if (designDisasterButtonLink) designDisasterButtonLink.value = product.designExpertContent.buttonLink;

    // Quiz Promo section
    const quizPromoHeading = document.getElementById('quizPromoHeading');
    const quizPromoButtonText = document.getElementById('quizPromoButtonText');
    const quizPromoButtonLink = document.getElementById('quizPromoButtonLink');
    if (quizPromoHeading) quizPromoHeading.value = product.quizPromoContent.heading;
    if (quizPromoButtonText) quizPromoButtonText.value = product.quizPromoContent.buttonText;
    if (quizPromoButtonLink) quizPromoButtonLink.value = product.quizPromoContent.buttonLink;
...
function previewQuiz() {
    ...
    html += `
        <div style="margin-bottom: 40px; padding: 20px; background: #f9f9f9; border-radius: 8px;">
            <h2 style="color: #333; text-align: center; margin-bottom: 20px;">${quizData.showroomSection.heading}</h2>
        ...
        q.options.forEach(opt => {
            html += `
                <div style="border: 1px solid #ddd; padding: 10px; border-radius: 5px; text-align: center;">
                    ${opt.image ? `<img src="${opt.image}" style="width: 100%; height: 100px; object-fit: cover; border-radius: 5px; margin-bottom: 10px;">` : '<div style="height: 100px; background: #f0f0f0; display: flex; align-items: center; justify-content: center; border-radius: 5px; margin-bottom: 10px;">No Image</div>'}
