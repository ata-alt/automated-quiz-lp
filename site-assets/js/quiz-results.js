...
async function loadResults(page = 0, limit = currentLimit, productFilter = currentFilter) {
    try {
        ...
        if (data.success) {
            ...
            console.log('[Results] Results loaded successfully:', data.results.length, 'results');
        } else {
    ...
    if (!results || results.length === 0) {
        container.innerHTML = `
            <div style="text-align: center; padding: 40px; color: #666;">
                ...
                <p>No one has submitted the quiz yet${currentFilter ? ' for the selected product' : ''}.</p>
            </div>
        `;
        return;
    }
    ...
    results.forEach(result => {
        ...
        tableHTML += `
            <tr>
                ...
                <td>${result.product_key ? escapeHtml(result.product_key) : '-'}</td>
                ...
                <td>
                    <button class="btn btn-sm" onclick="viewAnswers(${result.id})" style="padding: 4px 8px; font-size: 12px;">👁️ View</button>
                    <button class="btn btn-danger btn-sm" onclick="deleteResult(${result.id})" style="padding: 4px 8px; font-size: 12px; margin-left: 5px;">🗑️</button>
                </td>
            </tr>
        `;
    });
...
function renderPagination(pagination) {
    ...
    if (endPage < totalPages - 1) {
        ...
        paginationHTML += `<button class="pagination-btn" onclick="loadResults(${totalPages - 1})">${totalPages}</button>`;
    }
...
async function loadProductFilter() {
    try {
        ...
        if (data.success) {
            const productKeys = [...new Set(data.results.map(r => r.product_key).filter(Boolean))];
    ...
    let answersHTML = `
        <div class="user-info-card">
            <h3 style="margin: 0 0 16px 0; color: #1f2937; font-size: 18px; display: flex; align-items: center; gap: 8px;">
                <span style="background: #2563eb; color: white; width: 28px; height: 28px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 14px; font-weight: 600;">#${result.id}</span>
                Submission Details
            </h3>
            <div class="user-info-grid">
                <div class="user-info-item">
                    <span class="user-info-label">👤 Full Name</span>
                    <span class="user-info-value">${escapeHtml(result.name)}</span>
                </div>
                <div class="user-info-item">
                    <span class="user-info-label">✉️ Email Address</span>
                    <span class="user-info-value">
                        <a href="mailto:${escapeHtml(result.email)}" style="color: #2563eb; text-decoration: none;">
                ...
                <div class="user-info-item">
                    ...
                    <span class="user-info-value">${result.phone ? escapeHtml(result.phone) : 'Not provided'}</span>
                </div>
                <div class="user-info-item">
                    <span class="user-info-label">🛋️ Product Interest</span>
                    <span class="user-info-value">
                        ${result.product_key ?
                            `<span style="background: #f3f4f6; padding: 4px 8px; border-radius: 6px; font-size: 14px;">${escapeHtml(result.product_key)}</span>` :
                            'Not specified'}
                    </span>
                </div>
                <div class="user-info-item">
...
function exportResults() {
    ...
    allResults.forEach(result => {
        const submittedDate = new Date(result.created_at).toISOString().split('T')[0];
        const answersText = result.quiz_answers ?
            result.quiz_answers.map(a => `Q: ${a.questionText} A: ${a.optionText}`).join(' | ') : '';

        csv += `${result.id},"${escapeCSV(result.name)}","${escapeCSV(result.email)}","${escapeCSV(result.phone || '')}","${escapeCSV(result.product_key || '')}","${submittedDate}","${escapeCSV(answersText)}"\n`;
    });
